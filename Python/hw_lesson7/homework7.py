...
results = []
input_gen = input("Введите количество генерируемых пословиц (максимум - 46 пословиц.)\n")
if not input_gen.isdigit():
    print("Вы ввели не число. Перезапустите программу и поробуйте ещё раз.")
    input("Нажмите enter для закрытия программы.\n")
    quit()
input_gen = int(input_gen)

for i in range(input_gen):
    if len(variants) > 0:
        proverbs_take = proverbs.pop(random.randint(0, len(proverbs)-1))
        variants_take = variants.pop(random.randint(0, len(variants)-1))
        if "Умом" in proverbs_take:
            result_take = proverbs_take.replace("Умом", variants_take)
        else:
            result_take = proverbs_take.replace("Ум", variants_take)
        results.append(result_take)
    else:
        print("Достигнут лимит пословиц. Заканчиваю генерацию.")
        break
print("Генерация закончена. Вывожу сгенерированные пословицы:")
print(*results, sep='\n')
